...
describe("personImageURL", () => {
  it("builds skin + version query params", () => {
    expect(
      api.personImageURL(7, "headshot", { skin: "broadcast", version: 42 }),
    ).toBe("/api/v1/people/7/image/headshot?skin=broadcast&v=42");
  });
  ...
  it("omits the query entirely with no opts", () => {
    expect(api.personImageURL(9, "poster")).toBe(
      "/api/v1/people/9/image/poster",
    );
  });

  it("omits v when version is 0 (an unfilled slot)", () => {
    expect(
      api.personImageURL(1, "headshot", { skin: "brutalist", version: 0 }),
    ).toBe("/api/v1/people/1/image/headshot?skin=brutalist");
  });
});
...
describe("personGalleryImageURL", () => {
  it("stamps version and skin when present", () => {
    expect(
      api.personGalleryImageURL(5, 88, { version: 88, skin: "broadcast" }),
    ).toBe("/api/v1/people/5/images/88?skin=broadcast&v=88");
  });
...
describe("person source-of-truth clients (F37)", () => {
  ...
  it("setPersonFieldDecision PUTs the person decision path with the record source", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "PUT",
        body: JSON.stringify({ source: "record" }),
      }),
    );
  });
  ...
  it("renameEntity(person) surfaces the colliding person on 409 instead of throwing", async () => {
    ...
    await expect(
      api.renameEntity("person", 3, "Existing Person"),
    ).resolves.toEqual({
      conflict: colliding,
    });
  });
  ...
  it("renameEntity(person) throws ApiError on other failures", async () => {
    stub(400);
    await expect(api.renameEntity("person", 3, "x")).rejects.toMatchObject({
      status: 400,
    });
  });
});
  ...
  it("a normal 200 is unaffected by redirect:manual", async () => {
    ...
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          new Response(JSON.stringify({ owner: true }), { status: 200 }),
        ),
    );
...
describe("entity alias clients", () => {
  ...
  it("addEntityAlias POSTs the alias and returns the updated list", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({ alias: "J Law" }),
      }),
    );
...
describe("film enrichment clients (F59)", () => {
  ...
  it("ENRICH_ENTITY_BASE maps film to /films, alongside the three older kinds", () => {
    ...
    expect(Object.keys(ENRICH_ENTITY_BASE).sort()).toEqual([
      "film",
      "person",
      "studio",
      "video",
  ...
  it("runEnrichRefreshAll opens the picker on a needs_review film result", async () => {
    const fetchMock = stub(200, {
      results: [{ provider: "tmdb", status: "needs_review" }],
    });
    const opened: string[] = [];
  ...
  it("runEnrichRefresh surfaces the 503 body line for a paused provider", async () => {
    stub(503, {
      error: "tmdb is rate-limiting — try again in 17 s",
      provider: "tmdb",
      retry_after: 17,
    });
    const errors: string[] = [];
