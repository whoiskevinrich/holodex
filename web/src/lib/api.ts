...
const getAuthed = <T>(path: string): Promise<T> =>
  get<T>(path, fetch, { credentials: CREDS });
...
function uploadEntityImage(
  ...
  return uploadAuthed<{ id: number; version: number }>(
    `/${kind}/${id}/images/${role}`,
    form,
  );
}
...
export const api = {
  ...
  deleteVideoPoster: (id: number) =>
    sendAuthed<Record<string, never>>("DELETE", `/media/${id}/poster`),
  ...
  reorderPersonImages: (id: number, order: number[]) =>
    sendAuthed<{ images: PersonImageSet }>(
      "POST",
      `/people/${id}/images/reorder`,
      { order },
    ),
  ...
  listPeople: (
    ...
    for (const canonical of missingFacet ?? [])
      p.append("missing_facet", canonical);
    const qs = p.toString();
  ...
  listCategories: (fetchFn?: typeof fetch) =>
    get<{ items: Category[] }>(`/categories`, fetchFn),

  getCategory: (id: number, fetchFn?: typeof fetch) =>
    get<{ category: Category }>(`/categories/${id}`, fetchFn),

  createCategory: (name: string) =>
    sendAuthed<{ category: Category }>("POST", `/categories`, { name }),

  renameCategory: (id: number, name: string) =>
    sendAuthed<{ category: Category }>("POST", `/categories/${id}/rename`, {
      name,
    }),

  deleteCategory: (id: number) =>
    sendAuthed<Record<string, never>>("DELETE", `/categories/${id}`),

  assignCategoryTags: (id: number, tagIds: number[]) =>
    sendAuthed<{ category: Category }>("POST", `/categories/${id}/tags`, {
      tag_ids: tagIds,
    }),

  unassignCategoryTags: (id: number, tagIds: number[]) =>
    sendAuthed<{ category: Category }>("DELETE", `/categories/${id}/tags`, {
      tag_ids: tagIds,
    }),
  ...
  resolveOrCreateTag: (name: string) =>
    sendAuthed<{ tag: Tag }>("POST", `/tags`, { name }),
  ...
  getFilm: (id: number, fetchFn?: typeof fetch) =>
    get<FilmDetailResponse>(`/films/${id}`, fetchFn),
  ...
  bulkAttachFilmVideos: (
    filmId: number,
    videoIds: number[],
    startingSceneNumber: number | null,
  ) =>
    sendConflictable<
  ...
  cascadeFilmStudio: (
    id: number,
    req: { source: DecisionSource; manual_value?: string },
  ) =>
  ...
  facets: (fetchFn?: typeof fetch) =>
    get<{ facets: Facet[] }>(`/facets`, fetchFn),
  ...
  completenessFacets: (entityType: "video" | "person" | "studio") =>
    getAuthed<{ facets: FacetSummary[] }>(
      `/completeness/facets?entity_type=${entityType}`,
    ),
  ...
  completenessQueue: () =>
    getAuthed<{ groups: CompletenessFacetGroup[] }>(
      `/owner/completeness-queue`,
    ),
  ...
  dismissJobRun: (id: number) =>
    sendAuthed<{ dismissed: boolean }>(
      "POST",
      `/admin/activity/runs/${id}/dismiss`,
    ),
  dismissJobFailures: (days = 30) =>
    sendAuthed<{ dismissed: number }>(
      "POST",
      `/admin/activity/failures/dismiss`,
      { days },
    ),
  ...
  reloadConfig: async (): Promise<{ fields: number }> => {
    const body = await sendAuthed<{ fields?: number }>(
      "POST",
      `/admin/reload-config`,
    );
  ...
  enrichSources: () =>
    getAuthed<{ sources: EnrichSource[] }>(`/enrich/sources`),
  ...
  enrichResolve: (personId: number, provider: string, query: string) =>
    sendAuthed<{ candidates: EnrichCandidate[] }>(
      "POST",
      `/people/${personId}/enrich/resolve`,
      {
        provider,
        query,
      },
  ...
  enrichClear: (personId: number, provider: string) =>
    sendAuthed<Record<string, never>>(
      "DELETE",
      `/people/${personId}/enrich/${encodeURIComponent(provider)}`,
    ),
  ...
  enrichStudioResolve: (studioId: number, provider: string, query: string) =>
    sendAuthed<{ candidates: EnrichCandidate[] }>(
      "POST",
      `/studios/${studioId}/enrich/resolve`,
      {
        provider,
        query,
      },
  ...
  enrichStudioClear: (studioId: number, provider: string) =>
    sendAuthed<Record<string, never>>(
      "DELETE",
      `/studios/${studioId}/enrich/${encodeURIComponent(provider)}`,
    ),
  ...
  enrichFilmApply: (filmId: number, provider: string, externalId: string) =>
    sendAuthed<{
      ...
      year_collision?: FilmYearCollision;
    }>("POST", `/films/${filmId}/enrich`, {
      provider,
      external_id: externalId,
    }),
  ...
  enrichFilmClear: (filmId: number, provider: string) =>
    sendAuthed<Record<string, never>>(
      "DELETE",
      `/films/${filmId}/enrich/${encodeURIComponent(provider)}`,
    ),
  ...
  setFilmYear: (filmId: number, year: number) =>
    sendAuthed<{ year?: number; conflict?: FilmYearCollision }>(
      "PUT",
      `/films/${filmId}/year`,
      {
        year,
  ...
  addEntityAlias: async (
    ...
    return {
      aliases: ((await res.json()) as { aliases: PersonAlias[] }).aliases,
    };
  },
  ...
  deleteEntityAlias: (kind: EntityKind, id: number, aliasId: number) =>
    sendAuthed<Record<string, never>>(
      "DELETE",
      `/${ENTITY_BASE[kind]}/${id}/aliases/${aliasId}`,
    ),
  ...
  mergeEntities: (kind: EntityKind, canonicalId: number, fromId: number) =>
    sendAuthed<Record<string, unknown>>(
      "POST",
      `/${ENTITY_BASE[kind]}/${canonicalId}/merge`,
      {
        from_id: fromId,
      },
    ),
  ...
  denyTag: (term: string) =>
    sendAuthed<{ existing_tag?: boolean }>("POST", `/owner/tags/denylist`, {
      term,
    }),
  ...
  resolveExtractionReview: (
    ...
    sendAuthed<{ job_id?: number }>(
      "POST",
      `/owner/extraction-review/${id}/resolve`,
      {
        action,
        value,
      },
  ...
  extractAll: () =>
    sendAuthed<{ status: string; started: boolean }>(
      "POST",
      `/admin/extract-all`,
    ),
  ...
  revertWritebackBatch: (batchId: string) =>
    sendAuthed<{ job_ids: number[] }>(
      "POST",
      `/writeback/batches/${batchId}/revert`,
    ),
  ...
  enrichQueue: () =>
    getAuthed<{ rows: EnrichQueueRow[] }>(`/owner/enrich-queue`),
  ...
  trash: () =>
    getAuthed<{ items: TrashEntry[]; total: number }>(`/admin/trash`),
  ...
  syncTagWriteback: (id: number) =>
    sendAuthed<{ batch_id: string; enqueued: number }>(
      "POST",
      `/tags/${id}/writeback/sync`,
    ),

  setTagsWriteback: (tagIds: number[], enabled: boolean) =>
    sendAuthed<Record<string, never>>("PATCH", `/tags/writeback`, {
      tag_ids: tagIds,
      enabled,
    }),
