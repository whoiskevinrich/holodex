...
export const CORE_ROLES = [
  ...
  "banner",
  "poster",
] as const satisfies readonly Exclude<PersonImageRole, "extra">[];
...
export type DecisionSource =
  "file" | "record" | `provider:${string}` | "manual";
...
export interface EnrichCandidate {
  ...
  detail?: string[];
  // image_url is the provider's optional list-row thumbnail (F63, contract §2.3):
...
export interface ExtractionResult {
  ...
  fields: Array<{
    field: string;
    outcome: "noop" | "auto_applied" | "logged_only" | "queued";
  }>;
}
