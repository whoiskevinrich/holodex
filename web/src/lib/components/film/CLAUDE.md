...
| `EditSceneNumberDialog.svelte` | Edit-in-place scene number correction (design handoff §8, HOLODEX-326): one numeric field over `ConfirmDialog` chrome, mounted from both the film detail page's scenes grid and the media detail page's Films chip row — the caller supplies which name (video title or film name) to show and its own `onsave`/collision handling. Replaces detach+reattach for a simple renumber. |
| `sceneNumber.ts` | Shared scene-number field validation (`parseSceneNumberInput`) — the one positive-whole-number-or-blank rule `FilmAttachDialog` and `EditSceneNumberDialog` both need, kept in one place after `/simplify` caught it duplicated. |
