...
| `WritebackFormDialog.svelte` | Batch writeback modal, cockpit revision (HOLODEX-400, `docs/design/writeback-cockpit-handoff.md`): every replace field whose staged value differs from the file renders its candidate chooser — `curation/SourceChipRow` for short fields, `curation/SourceRadioList` for `long_text` — and the Write button is that chooser's Confirm (picks stage locally; `submit()` decides via `needsDecision`, then enqueues one fire-and-forget write, ADR-091). A row matching the file collapses to the `=` tier with a quiet `change` toggle; an `in_sync`-unknown row carries a `Can't verify` note. Row-class / staged-value / decision predicates are pure in `$lib/writebackCockpit.ts`. `image_url` rows keep HOLODEX-245's read-only comparison; merge rows keep their seeded text. |
