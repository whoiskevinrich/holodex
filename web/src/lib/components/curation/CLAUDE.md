...
| `SourceBadge.svelte` | Tier-2 per-field source-of-truth control (F56) — collapsed `ProvenanceBadge` at rest, click-to-expand `CurationChip` radio row with staged (not auto-committing) selection + explicit Confirm/Cancel. Used on Video (Metadata fields, incl. Overview), Person (all replace fields but name), and Studio (all replace fields but name). |
