...
| File | Purpose |
|---|---|
| `AutoFieldRows.svelte` | Read-only rows for display-only auto-registered non-canonical fields, shared by video/person/studio detail pages. Owner also sees a "Promote" pill opening `PromoteFieldEditor`. |
