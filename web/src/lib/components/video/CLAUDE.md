...
| `VideoGrid.svelte` | Responsive video grid — `min(density, capForWidth(viewport))` columns, 1 on a phone up to 16 on an ultrawide; card aspect ratio driven by `data-layout`. |
