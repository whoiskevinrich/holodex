...
| `SortDropdown.svelte` | Media sort `<select>`; options/order come from the single source of truth in `lib/filters.ts`. Generic over the bound value; an optional `extra` list prepends page-specific entries (the playlist page's `manual` — F69) so a page never forks the dropdown to add one. |
