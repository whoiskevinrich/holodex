...
func TestPath_ServerAssignedIDsOnly(t *testing.T) {
	got := entityimage.Path("/data/entity-images", 42, 7)
...
func TestStoreRemove_RoundTrip(t *testing.T) {
	...
	got, err := os.ReadFile(entityimage.Path(dir, 3, 9))
	if err != nil {
	...
	if _, err := os.Stat(entityimage.Path(dir, 3, 9) + ".tmp"); !os.IsNotExist(err) {
