...
type Person struct {
	...
	// search results (the row must show what matched) and on the video-detail and
	// film Cast grids (HOLODEX-461); omitted elsewhere, and the person detail page
...
// A future enrichment writer (HOLODEX-284) would add a "provider:<name>" source
// alongside it, per film_images' schema comment — not "enrichment" like Studio,
// since film_images can hold more than one provider-sourced row per role.
const FilmImageSourceUpload = "upload"

// ValidFilmImageRole reports whether role is one of the two known roles — the enum
// every request value is validated against (never a filesystem path).
