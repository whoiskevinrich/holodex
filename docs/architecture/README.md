...
- [Runtime owner settings (F40)](../specs/runtime-settings.md) — owner-editable settings at runtime; DB override over startup config; debuts `person_gallery_max` (ADR-059)
- [WebP provider images (F41)](../specs/gallery-webp.md) — accept provider WebP through the existing person-image re-encode gauntlet (no ADR; no contract change)
