...
5. [x] **Implementation (HOLODEX-254)** — `enrich.Source.SearchPattern` + `fileConfig.DefaultSearchPattern` (+ config-load validation, logged/dropped on an unknown token); `enrich.Manifest.PreferredSearchPattern` cached in-memory per provider (`Service.persistPreferredPattern`/`PreferredSearchPattern`, refreshed opportunistically on every owner-initiated `/describe`, mirroring the existing `FieldHints` cache lifecycle — no new DB table); `internal/enrich/query.go` (grammar, `ValidatePattern`, `QueryFields`, `sanitizeTitle`/`SanitizeTitle`, `Source.BuildQuery`); `getMedia` response gains `enrich_queries` (`internal/api` `buildVideoQueries`); the video-detail page passes `enrich_queries[provider] ?? displayTitle` into `EnrichPicker`'s existing `entityName` prop (confirmed zero `EnrichPicker.svelte` diff). Verified live against the AMV testbed with a real `search_pattern` configured — the picker seeded correctly and F22.5b's auto-search-on-open fired against it unchanged.
   **Disclosed scope trim:** refresh-all's `enrichQueryHint` (the one other raw-title call site, `internal/api/enrich_review.go`) gets D4's unconditional sanitizer (`enrich.SanitizeTitle`) but **not** per-provider D2 pattern rendering — it builds one hint shared across every provider in that concurrent fan-out, and giving each provider its own pattern there would mean restructuring the fan-out to build the hint inside each goroutine. The interactive owner-facing picker flow (`getMedia` → `enrich_queries` → `EnrichPicker`), which is what the spec's acceptance criteria and the design handoff's content-spec table concretely describe, has the full D1–D5 implementation. Follow-up if real friction shows up: extend `refreshOneProvider` to build its own hint per `src`.
6. [x] Provider-contract spec (`docs/specs/metadata-provider-contract.md`) §2.2 — documented `preferred_search_pattern` (table row + example JSON), a `hint.query` cross-reference in §2.3, and a new §4.9 deep-dive section mirroring §4.7/§4.8's style (grammar, token-value table, operator-override-always-wins note, "omitting this key is fully conformant" guidance).
