...
| 2.5a | `[smoke]` After 2.5, look at the Metadata list below the panel. | The written field now carries a `filename` source badge, sitting alongside any `tmdb`- or `file`-sourced fields. |
...
| 3.6a | `[agent]` On a video that is also TMDB-linked and whose filename supplies a field TMDB also supplies, resolve the extraction row, then click that field's source badge. | The chip row offers `file`, `filename`, and `tmdb` as three peers. `filename` is a normal namespace — no bespoke conflict UI was introduced. |
...
| 4.7a | `[human]` After confirming a write, look down the Metadata list for the field you just changed. | You can see where the value ended up, and a small label tells you it came from the filename — the same way other fields are labelled as coming from TMDB or from the file. You should never be left wondering whether anything happened. |
