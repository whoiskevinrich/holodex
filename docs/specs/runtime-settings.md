...
**Owner**: Project owner
**Date**: 2026-07-05
**Feature block**: **F41** — let the owner change selected operator settings **from the UI, at
...
- **A general config editor.** F41 exposes only keys on the in-code registry (debut: one). Arbitrary
...
- **Rewriting `holodex.yaml` from the app.** The override lives in the DB, separate from the operator's
  deploy config. *(Why: avoids racing env precedence and mutating read-only mounts — ADR-060 Option C.)*
- **Env as a runtime lock.** An explicitly-set env var seeds and resets but does **not** beat a written
  override. *(Why: single owner sets both; the UI is the most recent specific intent — ADR-060.)*
- **Raising per-enrich image yield.** F41 lets the *stored accumulation* cap grow; a single provider
...
table (ADR-060 §1). Additive; `.down.sql` drops it. **No seed rows** — an absent row means "no
...
Effective value = `DB override ?? config seed (yaml/env) ?? built-in default` (ADR-060 §2). The
...
validation (`min`/`max`/enum), and UI `label`/`help` (ADR-060 §4). **Debut registry has exactly one
...
- **P1-a** — a second registry entry (e.g. `card_layout`) wired to prove the seam with a non-int
  (enum) type. *Not required for F41 to ship; it is the proof the mechanism generalized.*

### Future Considerations (P2)

- **P2-a** — per-key `locked` flag in the registry so a hosted/multi-tenant build could let env pin a
  value against UI edits (ADR-060 sub-decision escape hatch). Design the registry struct so this is
  additive.
...
- **[engineering] Sidecar per-enrich yield** — F41 caps *accumulation*, not per-enrich yield. Should a
  ...
  non-blocking for F41. *(Resolved as out-of-scope here; tracked separately.)*
