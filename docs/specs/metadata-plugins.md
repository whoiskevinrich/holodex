...
4. **Stale enrichment (product).** Provider data can change upstream. Is `fetched_at` + a manual "re-enrich" button enough, or do we want a staleness indicator? (Leaning: manual only, on-demand ethos.) — *The manual re-enrich button is now specced as part of the per-item Refresh Metadata action; see [F29 / metadata-refresh.md](metadata-refresh.md). A staleness indicator remains deferred (F29.12).*
