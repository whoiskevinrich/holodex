...
| F65.3 | **Optional demotions.** The facets marked *Optional (F65)* in § Facet tables carry `optional` criticality: listed in the panel payload, never scored, never missing, never queued, never in the "Missing facet" chip. | The video "Missing facet" chip offers exactly `title, studio, actors, poster_url, overview, release_date, genres, external_provider_id`; a video with no `tagline` has no `tagline` entry in any queue group. |
