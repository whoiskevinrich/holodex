...
- **Multi-provider (§8)**: the control offers one `Adopt` per **matched** provider (a provider with no value for the field is omitted); two providers disagreeing on a replace field surfaces the **informational** "providers differ" hint — never a `text-warn` signal (only out-of-sync is warn); a global inter-provider trust order decides the *undecided* winner among providers, file still ahead of all.
...
- **Deferred (tracked)**: the browse **facet** entity-switch (HOLODEX-120: `?studio_id` + a Studios dropzone replacing the legacy `?studio=` string facet), the **TMDB company enrichment** slice (HOLODEX-121, S3), and **studio external-id dedup** (HOLODEX-122: capture the TMDB company id → `studio_external_ids` join for deterministic refresh/hints/dedup, extends ADR-053) — the legacy `?studio=` string filter stays working meanwhile.
