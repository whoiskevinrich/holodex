...
- **API** (`POST /media/{id}/refresh`): owner-gated (**401/403** without owner auth), **404** unknown, **409** soft-deleted, **503** when the service is unwired, **202** + report on success; per-item path mirrors `/media/{id}/enrich` + `/writeback`. *(Handler-level auth/status integration test is a tracked gap — the service-level invariants above are covered by unit tests.)*
