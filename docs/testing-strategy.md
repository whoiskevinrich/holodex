...
| **Provider asset URLs / `asset_hosts`** (ADR-039) | Unit | Asset-fetch allowlist = `base_url` host ∪ operator `asset_hosts`: an allowlisted CDN host over **https** is fetched, an **unlisted host is refused**, a cross-host asset over **plain http is refused** (https required off the base_url host), the base_url host over http still works; a provider response **cannot add a host**; `asset_hosts` **normalized** (trim/lowercase/de-empty) at config load; **first-success-per-role dedup** — a fallback asset fills a role only when the preferred one fails | ~95% |
...
- **Repo `person_images`**: insert/get/list/delete CRUD; **core-slot uniqueness** (second `headshot` upsert *replaces*, never two); **20-extra gallery cap** enforced transactionally (21st `extra` rejected; core slots never blocked by the cap); `sort_order` reorder; **`ON DELETE CASCADE`** with the person; the row-`id`-as-version invariant (replace ⇒ new id).
...
- **Enrichment asset download** (extends F22; host allowlist per ADR-039): a provider-returned asset URL is fetched **through the SSRF guards** — host allowlist = `base_url` ∪ operator `asset_hosts`, **https-for-cross-host**, cross-host-redirect refusal, response-size cap, timeout — then run through the **same normalization** and stored with provenance (`source=enrichment`, provider/external_id); a hostile asset URL (unlisted/internal host, cross-host http, redirect-to-internal, oversized body, non-image) is refused with nothing written. Multiple assets for one role are **preference-ordered, first success wins**.
