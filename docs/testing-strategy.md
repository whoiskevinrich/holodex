...
| **`ResolveOrCreateTag` / `POST /tags`** (HOLODEX-240, ADR-078, S5) | Unit + Integration | Backs `/categories/{id}`'s "+ Add tag" control — resolve-or-create **with no video attach**, sharing `resolveOrCreateByName`'s deny-list (422)/length-cap (400)/category-collision (409) checks with `AttachTagToVideo`; idempotent on a case/whitespace-variant re-resolve (same id, no duplicate); owner-gated (401 without token). **Known edge case, not a bug**: a zero-video tag created this way is invisible to `ListTags`/`GET /tags` (an inner join with `video_tags`) — it exists (`TagIDByName` resolves it, `GET /tags/{id}` 200s) and shows in its category's member-tag chips (`TagsForCategory` has no such join), but won't appear on `/tags`, in the merge picker, or in search until some video is tagged with it. Flagged in §11, not silently masked | ~90% |
...
(`docs/design/tag-category-create-affordance-handoff.md`) landed 2026-07-31. **UI-only by design**: both
backend endpoints (`POST /tags`, `POST /categories`) are unchanged and already carry the test coverage
HOLODEX-240's own S4 work and this file's earlier update gave them (§4 rows above), so this feature adds
**zero** new backend test surface. The frontend layer (§5 row above, plus the tag/category-creation-
asymmetry invariant above) is written ahead of implementation — **nothing under this ticket is built
yet**, so the §5 row and the invariant are the *target*, not yet exercised. The handoff's own flagged
implementation gotcha — the empty-state branch currently skips the pill grid `<div>` entirely — is the one
regression risk worth pinning a test to before calling this done.
...
- **`ResolveOrCreateTag`'s zero-video-tag visibility gap** (HOLODEX-240, discovered while closing the S5 test gap above): a tag created via `/categories/{id}`'s "+ Add tag" control with a brand-new name (not an existing tag) gets zero video associations, and `ListTags` inner-joins `video_tags` — so that tag is **invisible** on `/tags`, in the merge picker, and in search until some video is later tagged with it. It's still reachable via the category's own member-tag chip list (`TagsForCategory` has no such join) and by direct id (`GET /tags/{id}`). Not a data-integrity bug (the tag exists and is correctly linked) but a real UX surprise — "I added a tag to my category and it disappeared from Tags" — worth a product decision (e.g. `ListTags` could `LEFT JOIN`, or the add-tag control could warn on a brand-new name) rather than a silent fix bundled into a testing-strategy pass.
...
- **Tag & category create affordance (HOLODEX-243)**: spec + design handoff landed 2026-07-31; the §5 row and the tag/category-creation-asymmetry invariant above are written ahead of implementation — none of it is built or automated yet. UI-only (no backend change, no new backend test surface). Not a silent gap: tracked against the ticket's own gate checklist (spec done, design handoff done, testing strategy = this update, implementation still open).
