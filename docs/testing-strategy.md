...
- **A collapsed `detail` row costs exactly one text line** (F61, design handoff): 20 + 16 + 16 + 16 = 68px is the ceiling and the §12 bound; the toggle has no vertical padding **on purpose** (the caption's `+N more` idiom) because padding an inline-block button grows the line box. QA §4.6 (touch target) is the open human item — if `py-1` is ever added, the bound moves with it.
...
| `collapsed-detail-row-costs-one-line` | **pass** — 9 checks (F61, HOLODEX-380). Rides `enrich-picker-open:flood` unchanged: `flood`'s labels are distinct, so every row carries a `details` toggle and starts collapsed. `height >= 60 and <= 68` on `#enrich-opt-2` — the ceiling is label (20) + disambiguation (16) + actions line (16) + `py-2` (16), the floor is a row that lost its actions line (~51). **Mutation-tested**: replacing the `flex items-baseline` actions line with a bare `<div>` (the first cut's inline-block-on-the-baseline defect) lands the row at 76px in all nine cells. Skin-safe because Tailwind's line-heights are fixed px and the live run measured 66.6 in all three skins. The open-row invariants (lines in flow inside the scroller, no clipping at 25 × 8 lines) were measured live and are **not** asserted — they would need a preparation that opens toggles, and the in-flow `<ul>` inside the `<li>` has nothing to float (§12.2: not worth a harness change). |
