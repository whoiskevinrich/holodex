...
| **`StudiosForVideos`/`FilmStudios` icon + video-count extension** (HOLODEX-290, [design handoff](design/studio-link-card-handoff.md), planned) | Unit + Integration | Both queries widen their `SELECT` to `s.icon_url` plus a `video_studios`-scoped video-count aggregate, scanned into the already-declared `model.Studio.IconURL`/`VideoCount` fields — no schema/migration change. `TestStudiosForVideos_IncludesIconAndCount`/`TestFilmStudios_IncludesIconAndCount` assert both columns round-trip for a studio with an icon and a nonzero count, **and** that a studio with no icon (`icon_url` NULL) scans to Go's zero value (`""`) rather than erroring or panicking on the nullable column; the count reflects live `video_studios`/film-scene participation at query time, not a stored/stale counter. **Shared-query regression guard**: `StudiosForVideos` is also called by `mergeEntity`'s writeback propagation (`internal/api/entity_identity.go:185`), which reads only `.Name` off each result — the widened `SELECT` must not change that call's behavior, proven by re-running the existing merge-writeback test suite unmodified against the widened query rather than only adding new icon/count-specific assertions | ~90% |
