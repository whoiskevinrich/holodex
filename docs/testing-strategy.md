...
| **Completeness store — trigger set** (F65/HOLODEX-412, ADR-099 D4, migration 0048, `internal/db/completeness_triggers_test.go`) | Integration (migration) | The enumerating test the ADR calls for: `completenessInputs` lists every input table with one write each, and each write must leave exactly one `completeness_dirty` row for the owning entity (`TestMigration0048_EveryCompletenessInputDirtiesItsEntity`, 20 subtests); the table list is cross-checked **both ways** against the migration text's `CREATE TRIGGER … ON <table>` set, so an input table can be added neither to the migration without a test row nor to the test without a trigger (`TestMigration0048_TriggerSetMatchesEnumeratedInputs`); the `videos` update trigger fires only on a real change to `title`/`active`/`deleted_at` — a scan-shaped rewrite (`SET title = title, file_size = …`) dirties nothing, a soft-delete dirties the row (`TestMigration0048_VideoUpdateDirtiesOnlyOnScoredColumnChange`); trigger bodies survive the firing statement's conflict clause — an `INSERT OR REPLACE` and an `ON CONFLICT DO UPDATE` upsert against an already-dirty entity neither fail nor duplicate (`TestMigration0048_TriggersSurviveUpsertConflictPolicy`); deleting a `people` row drops its `entity_completeness`, `_missing` and dirty rows (`TestMigration0048_EntityDeleteDropsDerivedRows`); `video_people` / `video_studios` writes flag **both** sides — no person facet reads a link, but listability does, and a person with no active video has no store row, so re-linking must re-flag them (`TestMigration0048_LinkTablesDirtyBothSides`); shadow-table writes for an entity type nothing scores (a film's enrichment, a tag's alias) leave no dirty row at all (`TestMigration0048_UnscoredEntityTypesNeverDirty`) | 100% of the trigger set |
...
Then it is exactly 76px: label + disambiguation + one 24px actions line + py-2 (and, since F64,
     ...
     toggle losing its py-1 is now the text block's own 60px assertion, not this row's
```
...
| `candidate-slot-is-40-wide-on-every-row` | **pass** — 9 checks (F64, HOLODEX-406). Same prepared state; `applies: each`, `atLeast: 25`; `width >= 40 and <= 40` on `[role="listbox"] > li > div[aria-hidden="true"]` — every candidate row's thumbnail slot. This is the handoff's x-offset parity asserted by construction: the `<li>` is `flex items-start gap-3`, so text x = slot width + 12 on every row, and the harness has no cross-element metric to say it directly. **Mutation-tested**: dropping `w-10` fails every row — but only once the stub's portraits were 80 × 120; with 40 × 60 portraits the box borrowed the image's intrinsic width and the mutation passed. Not caught: the slot losing `aspect-[2/3]` on a pictured row (the image's own 2:3 supplies the 60) — a monogram row would show it and `flood` has none (§12.5). |
