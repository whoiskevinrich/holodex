...
- **Entity-generic seam (not built here)**: the decision store + primitive key by `(entity_type, entity_id, canonical_field)`; video-only is exercised now, but a unit asserts the resolver path is parameterized by entity (so the deferred People/Studio fast-follows ride the same tests with a different `BaselineSource`).
