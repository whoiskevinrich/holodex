...
**People images (F24, ADR-037)** — faces across the app; fully CI-testable, **no network** (the one external fetch is exercised against the in-process provider fake / a local test server, never a real host):
