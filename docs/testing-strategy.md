...
| **Year is an identity write, and a collision is atomic** (ADR-089 D3, HOLODEX-311, planned) | Integration | `films.year` is half `UNIQUE(name, year)` (migration 0043, films-entity RD8), so setting it from a provider's `release_date` is an identity change, not a field write. `TestFilmEnrichApply_SetsYearFromReleaseDate` — a clean apply sets the column and the resolved `release_date` in one commit. `TestFilmEnrichApply_YearCollisionRejectsAtomically` — **the load-bearing test**: with a second film already holding `(name, year)`, the apply leaves *both* films byte-identical, writes no `field_source_decisions` row, no `entity_enrichment` row and no `film_images` asset, and returns the occupying film's name and id. Asserting only "returns an error" would pass against a half-written state, which is the actual failure mode when the uniqueness check sits outside the commit. `TestFilmFields_NameHasNoProviderSource` pins the other half of the identity pair: `filmFields` synthesizes `name` from the baseline source alone, and a future provider source appended to it must fail this test loudly rather than quietly enabling an ungated rename (ADR-089 D3, spec Non-Goal 1) | ~95% |
...
- **A film identity collision changes nothing (F59/ADR-089 D3)**: a provider apply whose year would violate `UNIQUE(name, year)` leaves both films byte-identical — no decision, no enrichment row, no asset — and names the occupant. A rejected apply must never half-write.
