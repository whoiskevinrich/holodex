...
- **Deferred (tracked)**: the browse **facet** entity-switch (`?studio_id` + a Studios dropzone replacing the legacy `?studio=` string facet) and the **TMDB company enrichment** slice (S3) — the legacy `?studio=` string filter stays working meanwhile.
