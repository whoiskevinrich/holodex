...
Then it is between 60 and 68px: the ceiling is label + disambiguation + one actions line +
     py-2, the floor catches a row that lost its actions line. An inline-block button dropped
     straight into the <li> measures 74–76 — that regression is the reason the bound exists
```
