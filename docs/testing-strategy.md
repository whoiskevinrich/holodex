...
**Film-studio cascade — best-effort failure boundary, autostart hand-off, owner gating (F57, HOLODEX-285, ADR-086) — adversarial**
```
...
Given the cascade's POST response already carries a non-empty batch_id (jobs were enqueued
     synchronously as part of that same request, per ADR-086 D2)
